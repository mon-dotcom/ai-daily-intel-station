---
label: 02 / GitHub Heat Check
title: GitHub 昨天最值得關注的前十名 AI 開源項目
summary: 以表格整理可實際追蹤與評估的 AI 開源項目熱度。
layout: markdown
---
## GitHub 榜單示範

| Rank | 專案名稱 | 連結 | Star 數 | Trend | 專案類型 | 一句話特色 | 常見用途 | Beginner Fit |
| --- | --- | --- | ---: | --- | --- | --- | --- | --- |
| 1 | openmanus | [GitHub](https://github.com/mannaandpoem/OpenManus) | 42,000 | ▲ 上升 | Agent / Workflow | 多步驟任務代理框架 | 自動研究、工具串接 | 中高 |
| 2 | aider | [GitHub](https://github.com/Aider-AI/aider) | 29,000 | ▲ 上升 | Coding / DevTool | 終端 AI 協作寫程式 | 重構、除錯、修改程式 | 高 |
| 3 | ComfyUI | [GitHub](https://github.com/comfyanonymous/ComfyUI) | 51,000 | ■ 持平 | Video / Media | 節點式生成工作流 | 圖像、影片與工作流實驗 | 中 |

> 這種標準 Markdown 表格會被自動轉成網站的 table 區塊，不需要手寫 HTML。

### 三個重點觀察

1. Agent、Coding 與本地部署仍然是團隊最容易拿來試作的主流方向。
2. 影像與影片專案容易爆紅，但要判斷它是長期工具還是短期話題。
3. 若要讓團隊快速有感，先試低摩擦工具，比追最新 benchmark 更實際。

### Step-by-step 撰寫示範

::: steps
1. 先挑一個欄位最完整的專案加入表格。
2. 再補一段簡短觀察，說明它對團隊有沒有實際用途。
3. 如果你要示範操作流程，就用 `::: steps` 包住有序列表。
:::

### 圖片示範

![GitHub 專案榜單視覺示意](https://images.unsplash.com/photo-1555066931-4365d14bab8c?auto=format&fit=crop&w=1200&q=80 "圖片也能直接用 Markdown 寫，title 會變成圖說。")
